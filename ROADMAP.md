...
- ✅ **Dark/low-contrast photo reading FIXED** — replaced the single global threshold with (1) CLAHE local-contrast normalization on the aligned image and (2) a per-question *local* threshold that judges each bubble against the blank paper of its own row, so lighting gradients no longer defeat it. On the real shadowed phone photo, detection went 9 → 27 of ~30 marks with zero false positives on the blank columns and **no regression** on clean/flat/blank/synthetic sheets (identical counts old vs new). Remaining: in-app "good lighting, flat sheet" capture guidance (UX, not engine).
