...
- ⚠️ **Free-tier concurrency** — multiple scans grading at once on Render free (512 MB) can fail with `[Errno 11] Resource temporarily unavailable`; found during live testing. Serialize background grading (in-process queue/lock) or upgrade the tier.
